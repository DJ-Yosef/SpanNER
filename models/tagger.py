...
class BertNerTagger(LightningModule):
    ...
    def training_step(self, batch, batch_idx):
        ...
        labels = batch["labels"]  # 现在是span级别的标签 [batch_size, max_num_spans]
        ...
        # 确保logits和labels维度匹配
        batch_size, num_spans, n_class = logits.shape
        _, max_num_spans = labels.shape

        # 如果num_spans不等于max_num_spans，需要调整
        if num_spans != max_num_spans:
            # 可能需要截断或填充logits
            if num_spans < max_num_spans:
                # 填充logits
                pad_len = max_num_spans - num_spans
                logits = torch.cat([logits, torch.zeros(batch_size, pad_len, n_class)], dim=1)
            else:
                # 截断logits
                logits = logits[:, :max_num_spans, :]
