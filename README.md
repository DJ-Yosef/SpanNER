...
1. 最常见问题
   ...
   实体 start/end 索引越界
   字段缺失或 json 解析报错
2. 需要修改的主要文件| 作用             | 文件路径                        | 主要修改内容                                                              |
   | ---------------- | ------------------------------- | ------------------------------------------------------------------------- |
   | 数据预处理       | dataloader/dataload.py          | 调整读取/解析数据格式，保证每个样本都是 dict，且有 text和 entities 字段   |
...
dataprocess/wyd2span.py做了这个以及数据集划分7:1:1

将整个项目作为一个包来运行,在项目根目录下创建一个 `__init__.py` 文件，然后使用 `-m` 参数来运行模块
